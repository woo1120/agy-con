---
name: oma-gjc-ralplan
description: "Consensus planning with Planner/Architect/Critic loop. Forked from @gajae-code/coding-agent ralplan skill, adapted for OMA runtime."
...
```
/skill:oma-gjc-ralplan "task description"
```
...
```
/skill:oma-gjc-ralplan --interactive "task description"
```
...
8. *(--interactive only)* On approval: invoke `OMA orchestration workflow (/orchestrate or /ultrawork)` for execution -- never implement directly
...
| Gate does not fire on a vague prompt | The gate only catches prompts with <=15 effective words and no concrete anchors; add more detail or use `/skill:oma-gjc-ralplan` explicitly |
...
The consensus loop requires three agent roles. Read these from `resources/` before executing:

- **Planner**: `resources/planner-prompt.md` — read-only sequencing, acceptance criteria, risks
- **Architect**: `resources/architect-prompt.md` — read-only architecture review, severity rating
- **Critic**: `resources/critic-prompt.md` — read-only plan validation, OKAY/ITERATE/REJECT